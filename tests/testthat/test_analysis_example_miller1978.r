...
test_that("calculations of escalc() for measure='PFT' are correct.", {
   ...
   expect_that(round(dat$yi*2,4), is_equivalent_to(c(1.1391, 1.2888, 1.5253, 0.9515))) ### need *2 factor due to difference in definition of measure
   expect_that(round(dat$vi*4,4), is_equivalent_to(c(0.0870, 0.0571, 0.0465, 0.1538)))
...
test_that("results are correct for the fixed-effects model using unweighted estimation.", {
   ...
   expect_that(round(pred$pred, 4), is_equivalent_to(1.2262))

   pred <- predict(res, transf=transf.ipft.hm, targs=list(ni=dat$ni))
   expect_that(round(pred$pred, 4), is_equivalent_to(0.3164))
...
test_that("results are correct for the fixed-effects model using weighted estimation.", {
   ...
   pred <- predict(res, transf=function(x) x*2)
   expect_that(round(pred$pred, 4), is_equivalent_to(1.3093))

   pred <- predict(res, transf=transf.ipft.hm, targs=list(ni=dat$ni))
   expect_that(round(pred$pred, 4), is_equivalent_to(0.3595))
...
test_that("results are correct when there are proportions of 0 and 1.", {
   ...
   expect_that(transf.ipft(dat$yi, dat$ni), is_equivalent_to(c(0,1)))
...
test_that("back-transformations work as intended for individual studies and the model estimate.", {
   ...
   ### back-transformation of the individual outcomes
   expect_that(transf.ipft(dat$yi, dat$ni), is_equivalent_to(c(0.0, 0.4, 0.6, 0.8, 1.0)))

   ### back-transformation of the estimated average
   res <- rma(yi, vi, method="FE", data=dat)
   pred <- predict(res, transf=transf.ipft.hm, targs=list(ni=dat$ni))

   expect_that(round(pred$pred, 4), is_equivalent_to(0.6886))
   expect_that(round(pred$ci.lb, 4), is_equivalent_to(0.5734))
   expect_that(round(pred$ci.ub, 4), is_equivalent_to(0.7943))
