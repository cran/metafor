...
test_that("results are correct for the fixed-effects model.", {
   ...
   expect_that(round(c(as.matrix(coef(summary(res)))), 4), is_equivalent_to(c(0.1549, 0.0609, 2.5450, 0.0109, 0.0356, 0.2742)))
   expect_that(round(res$QE,4), is_equivalent_to(14.7640))
   expect_that(round(res$QEp,4), is_equivalent_to(0.0976))
...
test_that("results are correct for the random-effects model.", {
   ...
   ### compare with results on page 133 (Exhibit 7.3)
   expect_that(round(c(as.matrix(coef(summary(res)))), 4), is_equivalent_to(c(0.1534, 0.0858, 1.7893, 0.0736, -0.0146, 0.3215)))
   expect_that(round(res$tau2,6), is_equivalent_to(0.025955))
...
test_that("results are correct for the ANOVA-type analysis.", {
   ...
   expect_that(tmp[1,], is_equivalent_to(c( 0.2984, 0.0813,  0.1390, 0.4578)))
   expect_that(tmp[2,], is_equivalent_to(c(-0.0277, 0.0917, -0.2075, 0.1521)))
   expect_that(round(res$QM,4), is_equivalent_to(7.0739)) ### 7.0738 in chapter
   expect_that(round(res$QMp,4), is_equivalent_to(0.0078))
   expect_that(round(res$QE,4), is_equivalent_to(7.6901)) ### 7.6902 in chapter
   expect_that(round(res$QEp,4), is_equivalent_to(0.4643))
   expect_that(round(res0$QE,4), is_equivalent_to(6.4382)) ### 6.4383 in chapter
   expect_that(round(res0$QEp,4), is_equivalent_to(0.2659))
   expect_that(round(res1$QE,4), is_equivalent_to(1.2519))
   expect_that(round(res1$QEp,4), is_equivalent_to(0.7406))
...
test_that("results are correct for the meta-regression analysis (fixed-effects with moderators model).", {
   ...
   expect_that(round(coef(summary(res)),4), is_equivalent_to(expected))
   expect_that(round(res$QM,4), is_equivalent_to(7.0807))
   expect_that(round(res$QMp,4), is_equivalent_to(0.0290))
   expect_that(round(res$QE,4), is_equivalent_to(7.6832)) ### 7.6833 in chapter
   expect_that(round(res$QEp,4), is_equivalent_to(0.3614)) ### 0.3613 in chapter
...
test_that("results are correct for the meta-regression analysis (mixed-effects model).", {
   ...
   expect_that(round(coef(summary(res)),4), is_equivalent_to(expected))
   expect_that(round(res$QM,4), is_equivalent_to(5.5711)) ### 5.5709 in chapter
   expect_that(round(res$QMp,4), is_equivalent_to(0.0617))
   expect_that(round(res$tau2,5), is_equivalent_to(0.00488)) ### 5.5709 in chapter
...
test_that("results are correct for the comutation of R^2 via the anova() function.", {
   ...
   expect_that(tmp$R2, is_equivalent_to(81.2))
