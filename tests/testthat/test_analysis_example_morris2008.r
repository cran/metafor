...
test_that("calculations of escalc() are correct for measure='SMCR'.", {
   ...
   expect_that(round(datT$yi,4), is_equivalent_to(c( 0.5056, 1.0481, 1.8054, 1.4181,  0.0801)))
   expect_that(round(datT$vi,4), is_equivalent_to(c( 0.0594, 0.0254, 0.2322, 0.1225,  0.0802)))
   expect_that(round(datC$yi,4), is_equivalent_to(c(-0.2365, 0.0958, 0.0000, 0.2667, -0.4250)))
   expect_that(round(datC$vi,4), is_equivalent_to(c( 0.0544, 0.0173, 0.0511, 0.0232,  0.0864)))
   ...
   expect_that(round(dat$yi,2), is_equivalent_to(c(0.74, 0.95, 1.81, 1.15, 0.51)))

   ### (results for this not given in paper)
   expect_that(round(dat$vi,4), is_equivalent_to(c(0.1138, 0.0426, 0.2833, 0.1458, 0.1667)))
...
test_that("calculations of escalc() are correct for measure='SMCC'.", {
   ...
   ### (results for this not given in paper)
   expect_that(round(datT$yi,4), is_equivalent_to(c( 0.5417, 1.0198, 2.6619, 1.9088,  0.0765)))
   expect_that(round(datT$vi,4), is_equivalent_to(c( 0.0573, 0.0304, 0.5048, 0.2822,  0.0716)))
   expect_that(round(datC$yi,4), is_equivalent_to(c(-0.2213, 0.1219, 0.0000, 0.5575, -0.2126)))
   expect_that(round(datC$vi,4), is_equivalent_to(c( 0.0512, 0.0240, 0.1111, 0.1050,  0.0730)))

   ### compute difference between treatment and control groups
   dat <- data.frame(yi = datT$yi - datC$yi, vi = datT$vi + datC$vi)

   ### (results for this not given in paper)
   expect_that(round(dat$yi,2), is_equivalent_to(c(0.76, 0.90, 2.66, 1.35, 0.29)))
   expect_that(round(dat$vi,4), is_equivalent_to(c(0.1086, 0.0544, 0.6159, 0.3872, 0.1447)))
